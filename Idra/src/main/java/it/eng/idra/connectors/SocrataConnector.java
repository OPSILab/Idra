...
public class SocrataConnector implements IODMSConnector {
	...
	private JSONArray getJSONDatasets() throws JSONException, URISyntaxException, ODMSCatalogueOfflineException,
		...
		String returned_json = sendGetRequest1(node.getHost() + "/api/data.json");
		...
		logger.info("-- SOCRATA Connector Response - Result count:" + jsonArray.length());

		return jsonArray;
	}

	/**
	 * Performs mapping from Socrata JSON Dataset object to DCATDataset object
	 *
	 *
	 * @param node
	 *            Node which dataset belongs to
	 * @param d
	 *            Socrata Dataset to be mapped
	...
	public DCATDataset datasetToDCAT(Object d, ODMSCatalogue node) throws JSONException, ParseException {
		...
		// Properties to be mapped to Socrata metadata
	...
	/**
	 * Retrieves all datasets belonging to a federated SOCRATA node using SOCRATA
	 * API Client
	 * 
	 * 
	 * @param none
	...
	// GregorianCalendar(TimeZone.getTimeZone("UTC"));
	// oldDate.setLenient(false);
	// SimpleDateFormat socrataDateF = new
	...
	// newDate.setTime(socrataDateF.parse(newDatasets.get(newIndex).getDcat_modified().getValue()));
	//// oldDate.setTime(sdf.parse(oldDatasets.get(oldIndex).getDcat_modified().getValue()));
	//// oldDate.setTimeInMillis(sdf.parse(parseDate(oldDatasets.get(oldIndex).getDcat_modified().getValue())).getTime());
	////
	////// newDate.setTime(sdf.parse(newDatasets.get(newIndex).getDcat_modified().getValue()));
